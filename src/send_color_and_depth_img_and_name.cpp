...
// const int MY_BUFFER_SIZE = 5 * WIDTH * HEIGHT + 1; // 3 * 640 * 480 + 2 * 640 * 480;
const int MY_BUFFER_SIZE = 5 * WIDTH * HEIGHT + 1 + NAME_MAX_LEN;
...
class mySocketClient {
 ...
 public:
  ...
  int send( char* buffer, ros::Publisher pub ){
...
private:
  ros::NodeHandle _nh;
  ros::Subscriber _sub1, _sub2, _sub3;
  ros::Publisher pub;
...
public:
  ViewImage() {
    pub = _nh.advertise<std_msgs::String>("person_name_in", 1);
...
#endif
    _sub3 = _nh.subscribe ("/person_name_out", 1,  &ViewImage::name_cb, this);
    ROS_INFO ("Listening for incoming data on topic /person_name_out ..." );
  ...
  void name_cb( const std_msgs::StringConstPtr& msg ){
    // if( (msg->data.compare("NOBODY")!=0) && (msg->data.compare("SOMEBODY")!=0) )
    //   got_name_flg = true;
    // else
    //   got_name_flg = false;
    got_name_flg = true;
  ...
  void image_cb( const sensor_msgs::ImageConstPtr& msg ){
    ...
    buffer[ 5 * WIDTH * HEIGHT ] = (char)got_name_flg;
    memcpy( buffer + 5 * WIDTH * HEIGHT + 1, name.c_str(), NAME_MAX_LEN );
    msc.send( buffer, pub );
    got_name_flg = false; // reset.
  }
